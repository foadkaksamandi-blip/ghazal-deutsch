...
const report={version:"14.0.2",startedAt:new Date().toISOString(),checks:[],metrics:{},failures:[]};
...
(async()=>{
  ...
  assertCheck("pack-valid",product.validatePack(basePack,"14.0.2").valid,{});
  const invalids=[
  ...
  assertCheck("pack-invalid-rejection",invalids.every(p=>!product.validatePack(p,"14.0.2").valid),{cases:invalids.length});
  ...
  const auto=await qa.runAutomated(qaState,storage,{device:{version:"14.0.2",assetIntegrity:true,cryptoSelfTest:true,debuggable:false,cleartextDisabled:true}});
