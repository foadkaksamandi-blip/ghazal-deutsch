...
const report={version:"14.0.1",startedAt:new Date().toISOString(),checks:[],metrics:{},failures:[]};
...
(async()=>{
  ...
  assertCheck("pack-valid",product.validatePack(basePack,"14.0.1").valid,{});
  const invalids=[
  ...
  assertCheck("pack-invalid-rejection",invalids.every(p=>!product.validatePack(p,"14.0.1").valid),{cases:invalids.length});
  ...
  assertCheck("version-consistency",pkg.version==="14.0.1"&&gradle.includes('versionName "14.0.1"')&&main.includes('return "14.0.1"')&&bridge.includes('return activity == null ? "14.0.1"'),{pkg:pkg.version});
  ...
  const auto=await qa.runAutomated(qaState,storage,{device:{version:"14.0.1",assetIntegrity:true,cryptoSelfTest:true,debuggable:false,cleartextDisabled:true}});
