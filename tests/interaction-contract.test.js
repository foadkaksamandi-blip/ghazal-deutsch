...
test('physical-touch instrumentation covers onboarding navigation lesson modal and dynamic Stage 5/6 UI',()=>{
  ...
  for(const token of ['sendPointerSync',"[data-action='skip-placement']","[data-nav='path']","[data-nav='practice']","[data-nav='migration']","[data-nav='profile']","[data-action='open-lesson']","[data-r12='hub']"])assert.ok(testFile.includes(token),token);
});
