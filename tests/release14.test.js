...
test('Stage 8 final core locks version package and offline privacy model',()=>{
  ...
  assert.equal(core.VERSION,'14.0.1');
  assert.equal(core.VERSION_CODE,15);
...
test('Stage 8 runtime refuses a final build whose production identity does not verify',()=>{
  ...
  assert.ok(main.includes('return "14.0.1"'));
  assert.ok(bridge.includes('return activity == null ? "14.0.1"'));
});
...
test('Stage 8 package and final gate remain version-consistent',()=>{
  ...
  assert.equal(pkg.version,'14.0.1');
  assert.ok(gradle.includes('versionName "14.0.1"'));
  assert.ok(qa.includes('const VERSION="14.0.1"'));
  assert.ok(finalCore.includes('const VERSION="14.0.1"'));
});
